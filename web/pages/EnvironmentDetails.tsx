...
import { Stagelet, BuildJob } from "../types";
...
const EnvironmentDetails: React.FC = () => {
	...
	const [env, setEnv] = useState<Stagelet | null>(null);
			...
			<div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
				<div>
					<div className="flex items-center gap-3 mb-2">
						<h1 className="text-2xl font-bold text-white font-mono">{env.subdomain_hash}</h1>
						<StatusBadge status={env.status} />
					</div>
					<div className="flex items-center gap-4 text-sm text-zinc-400">
						<Link to={`/projects/${env.project_id}`} className="hover:text-white transition-colors">
							Project
						</Link>
						<span>•</span>
						<span>PR #{env.pr_number}</span>
						<span>•</span>
						<span className="font-mono">{env.commit_hash}</span>
						<span>•</span>
				...
				<div className="flex items-center gap-3">
						...
						<AlertDialogContent>
							<AlertDialogHeader>
								...
								<AlertDialogDescription>
									...
									<span className="font-mono text-white"> {env.subdomain_hash}</span> and remove all
									associated data.
								</AlertDialogDescription>
							</AlertDialogHeader>
							<AlertDialogFooter>
								...
								<AlertDialogAction
									onClick={() => {
										db.deleteStagelet(env.id);
										navigate("/");
									}}
								>
									Yes, terminate environment
								</AlertDialogAction>
							</AlertDialogFooter>
						</AlertDialogContent>
					</AlertDialog>
					...
					{env.status === "ready" && (
						<a
							href={`https://${env.subdomain_hash}.stagely.dev`} // This link won't work in reality
							target="_blank"
						...
						<div className="bg-zinc-900 border border-zinc-800 rounded-lg p-6 space-y-4">
							...
							<dl className="space-y-3 text-sm">
								<div className="flex justify-between">
									...
									<dd className="text-zinc-300 font-mono">{env.vm_ip || "Provisioning..."}</dd>
								</div>
								<div className="flex justify-between">
								...
								<div className="flex justify-between">
									...
									<dd className="text-zinc-300">{new Date(env.created_at).toLocaleString()}</dd>
								</div>
							</dl>
						</div>
						<div className="bg-zinc-900 border border-zinc-800 rounded-lg p-6 space-y-4">
							...
							<div className="flex items-center gap-2">
								<span className="text-3xl font-bold text-white">${env.estimated_cost_usd}</span>
				...
				{activeTab === "logs" && (
					<div className="bg-zinc-950 border border-zinc-800 rounded-lg overflow-hidden font-mono text-xs md:text-sm">
						<div className="bg-zinc-900 px-4 py-2 border-b border-zinc-800 flex items-center justify-between">
							<span className="text-zinc-400">build-{env.commit_hash.substring(0, 7)}.log</span>
				...
				{activeTab === "config" && (
					<div className="space-y-6">
						<EnvVarManager
							...
							description="Variables overridden for this specific environment."
						/>
						<EnvVarManager
							level="stagelet"
							referenceId={env.id}
							...
							description="Encrypted secrets overridden for this specific environment."
						/>
						...
						<div className="bg-zinc-900 border border-zinc-800 rounded-lg p-6 opacity-75">
							...
							<p className="text-sm text-zinc-500">
								This environment also inherits configuration from{" "}
								<Link to={`/projects/${env.project_id}`} className="text-blue-400 hover:underline">
									Project
								</Link>{" "}
								and{" "}
