...
Stagely is a self-hosted ephemeral stagelet platform that creates production-like testing stagelets based on GitHub Pull Requests. It provisions VMs across multiple cloud providers, acts as a CI/CD job runner, and provides a centralized dashboard for managing preview stagelets.
...
- **Ephemeral by Design**: Stagelets are automatically reaped when PRs close or TTL expires
...
- Serve "Building..." static page when stagelet is not ready
...
- Cached in memory
- No per-stagelet certificate generation (avoids Let's Encrypt rate limits)
...
if target == nil {
    return "404: Stagelet not found"
}

if target.Status == "building" {
    return staticHTML("Building your stagelet...")
}
...
- HTTPS only
- Images are namespaced by project: `registry.internal/{project_id}/{stagelet_id}:{tag}`
...
teams (id, slug, name)
  └─> projects (id, team_id, slug, repo_url, cloud_provider_config)
       └─> stagelets (id, project_id, pr_number, subdomain_hash, vm_ip, status)
            ├─> workflow_runs (id, stagelet_id, status, created_at)
...
- Encryption key stored in stagelet variable (or KMS)
...
- Stored in `stagelets.subdomain_hash` column

**Why Flat URLs:**
- Single wildcard certificate works for all stagelets
...
```
GitHub → Stagely Core webhook
  → Core marks stagelet as "terminated"
...
```sql
SELECT id, vm_id, provider FROM stagelets
...
For each stale stagelet:
- Call provider API to terminate VM
- Update stagelet status to "reaped"
- Delete Redis route

## Observability

### Metrics (Prometheus)
- Active stagelets count
- Build queue depth
...
### Alerts
- Stagelet stuck in "building" >10 minutes
...
- Revocable: Core can reject connection if stagelet deleted
